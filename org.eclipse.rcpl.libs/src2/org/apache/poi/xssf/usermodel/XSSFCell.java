 ...
 * conversions themselves. Formula cells have the formula string, as well as the
...
public final class XSSFCell implements Cell {
	...
	/**
	 * the xml bean containing information about the cell's location, value, data
	...
	/**
	 * Table of strings shared across this workbook. If two cells contain the same
	 * string, then the cell value is the same index into SharedStringsTable
	 */
	 ...
	 * <p>
	 * For strings, numbers, and errors, we throw an exception. For blank cells we
	 * return a false.
	 * </p>
	 * 
	 * @return the value of the cell as a boolean
	 * @throws IllegalStateException if the cell type returned by
	 *                               {@link #getCellType()} is not
	 *                               CELL_TYPE_BOOLEAN, CELL_TYPE_BLANK or
	 *                               CELL_TYPE_FORMULA
	 */
	@Override
	public boolean getBooleanCellValue() {
		...
		case CELL_TYPE_FORMULA:
			// YK: should throw an exception if requesting boolean value from a non-boolean
			// formula
			return _cell.isSetV() && TRUE_AS_STRING.equals(_cell.getV());
		default:
	...
	public XSSFRichTextString getRichStringCellValue() {
		...
		case CELL_TYPE_STRING:
			if (_cell.getT() == STCellType.INLINE_STR) {
				if (_cell.isSetIs()) {
					// string is expressed directly in the cell definition instead of implementing
	 ...
	 *            cached string result, for String cells we'll set its value. For
	 *            other types we will change the cell to a string cell and set its
	 *            value. If value is null then we will change the cell to a Blank
	 *            cell.
	 */
	@Override
	 ...
	 * Set a string value for the cell.
	 *
	 * @param str value to set the cell to. For formulas we'll set the
	 *            'pre-evaluated result string, for String cells we'll set its
	 *            value. For other types we will change the cell to a string cell
	 *            and set its value. If value is null then we will change the cell
	 *            to a Blank cell.
	 */
	@Override
	...
	public String getCellFormula() {
		...
		if (cellType != CELL_TYPE_FORMULA)
			throw typeMismatch(CELL_TYPE_FORMULA, cellType, false);
	...
	private String convertSharedFormula(int si) {
		...
		Ptg[] fmla = sf.convertSharedFormulas(ptgs, getRowIndex() - ref.getFirstRow(),
	 ...
	 * {@link #setCellValue(double)} or {@link #setCellValue(String)}
	 * </p>
	 *
	 * @param formula the formula to set, e.g. <code>"SUM(C4:E4)"</code>. If the
	 *                argument is <code>null</code> then the current formula is
	 *                removed.
	 * @throws                       org.apache.poi.ss.formula.FormulaParseException
	 *                               if the formula has incorrect syntax or is
	 *                               otherwise invalid
	 * @throws IllegalStateException if the operation is not allowed, for example,
	 *                               when the cell is a part of a multi-cell array
	 *                               formula
	 */
	@Override
	...
	/**
	 * Set the style for the cell. The style should be an XSSFCellStyle
	 * created/retreived from the XSSFWorkbook.
	 *
	 * @param style reference contained in the workbook. If the value is null then
	 *              the style information is removed causing the cell to used the
	 ...
	 *         {@link #CELL_TYPE_BOOLEAN}, {@link #CELL_TYPE_ERROR}) depending on
	...
	private CellType getBaseCellType(boolean blankCells) {
		...
		case STCellType.INT_N:
			if (!_cell.isSetV() && blankCells) {
				...
				// distinguish blank values which sometimes get translated into zero and
				// sometimes
	 ...
	 * @exception NumberFormatException if the cell value isn't a parsable
	 *                                  <code>double</code>.
	 * @see DataFormatter for formatting this date into a string similar to how
	 *      excel does.
	 */
	@Override
	...
	/**
	 * Set a date value for the cell. Excel treats dates as numeric so you will need
	 * to format the cell as a date.
	 *
	 * @param value the date value to set this cell to. For formulas we'll set the
	 *              precalculated value, for numerics we'll set its value. For other
	 *              types we will change the cell to a numeric cell and set its
	 *              value.
	 */
	@Override
	 ...
	 * to format the cell as a date.
	 * <p>
	 * This will set the cell value based on the Calendar's timezone. As Excel does
	 * not support timezones this means that both 20:00+03:00 and 20:00-03:00 will
	 * be reported as the same value (20:00) even that there are 6 hours difference
	 * between the two times. This difference can be preserved by using
	 * <code>setCellValue(value.getTime())</code> which will automatically shift the
	 * times to the default timezone.
	 * </p>
	 *
	 * @param value the date value to set this cell to. For formulas we'll set the
	 *              precalculated value, for numerics we'll set its value. For
	 *              othertypes we will change the cell to a numeric cell and set its
	 *              value.
	 */
	@Override
	 ...
	 * @throws IllegalStateException if the cell type returned by
	 *                               {@link #getCellType()} isn't CELL_TYPE_ERROR
	 * @see FormulaError
	 */
	 ...
	 * <p>
	 * For strings, numbers, and booleans, we throw an exception. For blank cells we
	 * return a 0.
	 * </p>
	 *
	 ...
	 *                  the precalculated value , for errors we'll set its value.
	 *                  For other types we will change the cell to an error cell and
	 *                  set its value.
	 * @see FormulaError
	 */
	@Override
	 ...
	 *              precalculated value , for errors we'll set its value. For other
	 *              types we will change the cell to an error cell and set its
	 *              value.
	 */
	 ...
	 * <p>
	 * If the cell contains a string, then this value is an index into the shared
	 * string table, pointing to the actual string value. Otherwise, the value of
	 * the cell is expressed directly in this element. Cells containing formulas
	 * express the last calculated result of the formula in this element.
	 * </p>
	 *
	...
	private static RuntimeException typeMismatch(int expectedTypeCode, int actualTypeCode, boolean isFormulaCell) {
		String msg = "Cannot get a " + getCellTypeName(expectedTypeCode) + " value from a "
	...
	private static void checkBounds(int cellIndex) {
		...
		if (cellIndex < 0 || cellIndex > maxcol) {
			throw new IllegalArgumentException("Invalid column index (" + cellIndex + ").  Allowable column range for "
					+ v.name() + " is (0.." + maxcol + ") or ('A'..'" + v.getLastColumnName() + "')");
		}
	}
	...
	/**
	 * Assign a comment to this cell. If the supplied comment is null, the comment
	 * for this cell will be removed.
	 *
	...
	/**
	 * Returns the xml bean containing information about the cell's location
	 * (reference), value, data type, formatting, and formula
	 *
	...
	/**
	 * Set a new internal xml bean. This is only for internal use, do not call this
	 * from outside!
	 * 
	 ...
	 * the cell a somewhat reasonable value until the setCellValue() call (if at
	 * all). TODO - perhaps a method like setCellTypeAndValue(int, Object) should be
