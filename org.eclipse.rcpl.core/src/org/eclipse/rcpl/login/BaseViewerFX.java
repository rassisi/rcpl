 ...
 * <p>
 * <a href=
 * "http://files.idrsolutions.com/samplecode/org/jpedal/examples/baseviewer/BaseViewer.java.html">Click
 * here for a bare-bones PDF Viewer in Java Swing.</a>
 * </p>
 * <p>
 * <p>
 * <a href=
 * "http://www.idrsolutions.com/java-pdf-library-support/http://www.idrsolutions.com/java-pdf-library-support/">For
 * more help and tutorials, visit our websites Support Section.</a>
 * </p>
 */
public class BaseViewerFX extends Application {
	...
	public enum FitToPage { // control how we fit the content to the page
		AUTO, // AUTO will automatically fit the content to the stage depending on its
				// orientation
	...
	public void start(final Stage stage) {
		...
		/*
		 * setup initial display Setting this before loadPDF() gives access to the
		 * toolbar buttons when called in loadPDF() via id.
		 */
	...
	/**
	 * creates all the components and adds change listeners for auto-centering for
	 * JavaFX PDF viewer
	 *
	...
	private ToolBar setupToolBar() {
		...
		if (!options.isEmpty()) {
			...
			transitionList.valueProperty().addListener(new ChangeListener<String>() {
				@Override
				public void changed(final ObservableValue<? extends String> ov, final String oldVal,
						final String newVal) {
	...
	private void openFile(final File input, final String url, final boolean isURL) {
		try {
			...
			// This code block deals with user input and JVM passwords in Encrypted PDF
			// documents.
			if (pdf.isEncrypted()) {
				...
				// While the PDF content is not viewable, repeat until the correct password is
				// found
	...
	private void showPasswordPrompt(final int passwordCount) {
		...
		// If the user has attempted to enter the password more than once, change the
		// text
		if (passwordCount >= 1) {
			...
			exitTransition.setOnFinished(new EventHandler<ActionEvent>() {
				...
				public void handle(final ActionEvent t) {
					...
					if (direction != TransitionDirection.NONE) {
						entryDirection = direction == TransitionDirection.LEFT ? TransitionDirection.RIGHT
								: TransitionDirection.LEFT;
					}

					final Transition entryTransition = FXViewerTransitions.entryTransition(pdf, transitionType,
							entryDirection);
