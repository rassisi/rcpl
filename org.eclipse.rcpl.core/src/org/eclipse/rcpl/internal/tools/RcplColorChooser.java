 ...
 *
 */
public class RcplColorChooser extends VBox {
	...
	public Color getChosenColor() {
		return valuePropertyWrapper.get();
	}
	...
	private static final String[][] webPalette = { { "aliceblue", "#f0f8ff" }, { "antiquewhite", "#faebd7" },
	...
	public RcplColorChooser(String[][] colors) {
		...
		chosenColorName.addListener(new ChangeListener<String>() {
			...
			public void changed(ObservableValue<? extends String> observableValue, String oldName, String newName) {
				if (newName != null) {
					...
					valuePropertyWrapper.set(Color.web(newName));
				}
			}
		});
		...
		for (String[] namedColor : colors) {
			...
			// todo it would be nice to be able to have the tooltip appear immediately on
			// mouseover, but there is no easy way to do this, (file jira feature request?)
			...
			// color the button appropriately and change it's hover functionality (doing
			// some of this in a css sheet would be better).
			...
			colorChoice.setOnMouseEntered(new EventHandler<MouseEvent>() {
				...
				public void handle(MouseEvent mouseEvent) {
					final String borderStyle = "-fx-border-color: ladder(" + colorHex
							+ ", whitesmoke 49%, darkslategrey 50%); -fx-border-width: 2;";
